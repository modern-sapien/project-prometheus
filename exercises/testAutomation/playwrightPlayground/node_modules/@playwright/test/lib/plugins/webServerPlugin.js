...
class WebServerPlugin {
  ...
  async teardown() {
    if (!this._childProcess || !this._childProcess.pid || this._childProcess.killed) return;
    // Send SIGTERM and wait for it to gracefully close.
    try {
      if (process.platform === 'win32') {
        const taskkillProcess = (0, _child_process.spawnSync)(`taskkill /pid ${this._childProcess.pid} /T /F`, {
          shell: true
        });
        const [stdout, stderr] = [taskkillProcess.stdout.toString(), taskkillProcess.stderr.toString()];
        if (stdout) debugWebServer(`[pid=${this._childProcess.pid}] taskkill stdout: ${stdout}`);
        if (stderr) debugWebServer(`[pid=${this._childProcess.pid}] taskkill stderr: ${stderr}`);
      } else {
  ...
  async _startProcess() {
    ...
    this._childProcess = (0, _child_process.spawn)(this._options.command, [], {
      env: {
    ...
    this._processExitedPromise = new Promise((resolve, reject) => {
      this._childProcess.once('exit', (code, signal) => resolve({
        code,
        signal
      }));
    });
    debugWebServer(`Process started`);
    this._childProcess.stderr.on('data', line => {
      var _onStdErr, _ref;
  ...
  async _waitForAvailability() {
    ...
    const {
      timedOut
    } = await Promise.race([(0, _timeoutRunner.raceAgainstTimeout)(() => waitFor(this._isAvailable, cancellationToken), launchTimeout), this._processExitedPromise.then(({
      code,
      signal
    }) => {
